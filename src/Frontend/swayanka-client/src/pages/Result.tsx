...
export const Result: React.FC = () => {
  ...
  return (
    <div className="min-h-screen pt-32 pb-12 bg-white px-4 animate-in fade-in slide-in-from-bottom-10">
      <div className="max-w-6xl mx-auto grid grid-cols-1 lg:grid-cols-2 gap-16 items-center">
        ...
        <div>
          <div className="flex items-center gap-3 mb-6"><span className="px-3 py-1 bg-gray-100 text-black rounded-full text-xs font-bold uppercase tracking-wide">Your Match</span><span className="text-gray-400 text-sm">{product.gender} • {product.fabric}</span></div>
          <h1 className="text-5xl font-black text-gray-900 mb-6 leading-tight">{product.name}</h1>
          <p className="text-xl text-gray-500 mb-8 leading-relaxed border-l-4 border-black pl-6">{product.description}</p>
          <div className="mb-10">
            <h3 className="text-sm font-bold text-gray-900 uppercase tracking-widest mb-4">Select Size</h3>
            <div className="flex gap-4">{['S', 'M', 'L', 'XL'].map(size => (<button key={size} onClick={() => setSelectedSize(size)} className={`w-16 h-16 rounded-2xl text-lg font-bold flex items-center justify-center transition-all border-2 ${selectedSize === size ? 'border-black bg-black text-white shadow-lg transform scale-110' : 'border-gray-100 text-gray-400 hover:border-black hover:text-black'}`}>{size}</button>))}</div>
          </div>
